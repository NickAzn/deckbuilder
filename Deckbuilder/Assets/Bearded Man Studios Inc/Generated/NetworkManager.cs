...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkBoardManagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkBoardManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkBoardManagerNetworkObject.Length > 0 && NetworkBoardManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkBoardManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkBoardManager instead, its shorter and easier to type out ;)")]
		public NetworkBoardManagerBehavior InstantiateNetworkBoardManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkBoardManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkBoardManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkBoardManagerBehavior>().networkObject = (NetworkBoardManagerNetworkObject)obj;
		...
		public NetworkBoardManagerBehavior InstantiateNetworkBoardManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkBoardManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkBoardManagerBehavior>();
